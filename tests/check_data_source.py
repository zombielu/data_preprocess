...
def get_related_trade_records(
    ...
    for _, row in tqdm(reference_df.iterrows(), total=len(reference_df)):
        ...
        if reference_date < datetime(2018, 4, 2, 0, 0, 0).date() \
                or reference_date > datetime(2025, 6, 30, 0, 0, 0).date():
            continue
        ...
        o, h, l, c, v = row['open'], row['high'], row['low'], row['close'], \
                        row['Volume']
        print(
            f'Reference open, high, low, close, volume values: {o, h, l, c, v}'
        )
        ...
        volume_sum = full_data.groupby('instrument_id')['size'].sum()
        max_instrument_id = volume_sum.idxmax()
        reference_contracts[reference_date] = [max_instrument_id]
        ...
        if max_ohlc >= 1:
            ...
            reference_contracts[reference_date].append(inst_id)
...
if __name__ == "__main__":
    reference_file_path = '/Users/weilinwu/Downloads/CME_MINI_ES1!, 1D_eb45c.csv'
    input_folder_path = '/Users/weilinwu/Downloads/raw_data/Archive'
